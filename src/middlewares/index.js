  ...
  try {
    ...
    if (!idToken || !idToken.startsWith("Bearer ")) {
      return utils.sendError(
        res,
        404,
        "Token is not available or is Invalid."
      )();
    }
    ...
    if (exception.message === "jwt expired") {
      return utils.sendError(
        res,
        500,
        `Auth Token Expired in ${exception.expiredAt}`
      )();
    }
...
const createAuthToken = async (user, expire = "365d") => {
  const { email, userName, _id, role } = user;

  return await jwt.sign({ email, userName, _id, role }, config.JWT_SECRET, {
    expiresIn: expire,
  });
};
...
const verifyAuthToken = async (idToken) => {
  try {
    ...
    if (userData && userData.isActive == false)
      throw "Your Account is Locked By Admin";
...
export { me, createAuthToken, verifyAuthToken };
